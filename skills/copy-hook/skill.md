...
1. Read `brand.md` (for brand voice, tone, and audience) and `likes.md` (specifically the `Copywriting & Hooks` and `Avoid / Disliked` sections).

2. Identify the target post topic from the user's prompt or the active item in `ideas.md`.

3. Craft the **On-Card Visual Copy** (text that will physically render inside the HTML/CSS graphic):
...
4. Craft the **Instagram Post Caption**:
...
5. Determine the post directory: `posts/YYYY-MM-DD_<topic-slug>/`. Create the directory if it does not exist, and write the caption to `caption.md`:
   ```markdown
   # Caption: [Topic Title]
...
6. Output the copy for the user to review. Proceed to `/design-html` once confirmed.
