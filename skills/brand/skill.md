...
1. Check if `brand.md` exists in the workspace root.
...
2. Conduct the 4-part onboarding interview. Ask questions sequentially or in focused batches, giving concrete examples:
   ...
   - **Part 2: Aesthetic Vibe**: Choose a distinct design archetype (e.g., Swiss Typography, Brutalist Cyberpunk, Minimalist Editorial, Luxury Darkroom, Vibrant Pop, Modern Monochromatic).
   - **Part 3: Visual Assets & Palette**:
     - *Colors*: 1 primary background, 1-2 foreground/text colors, 1 vibrant accent. Record exact hex/hsl codes.
     ...
     - *Visual Treatments*: Grain textures, glassmorphism blur, sharp borders, glow effects, or flat minimalism.
...
3. When the user's answers are vague, contradictory, or generic, invoke the `/grill-me` skill pattern to stress-test decisions before writing the file.

4. Write the approved brand specification to `brand.md` in the workspace root using this structure:
   ```markdown
...
5. Check if `likes.md` exists. If not, initialize `likes.md` with empty categorized sections ready to record future post feedback.
